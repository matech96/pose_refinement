...
def create_prediction_viz_frames(gt, pred, valid, out_path, draw_func=None, viewpoint_frame=0):
    """
    ...
    Parameters:
        ...
        viewpoint_frame: the frame used for setting the camera position. The camera will point at the center of the average GT pose
    """
    ...
    plt.clf()
    ax = get_3d_axes()
    xroot, yroot, zroot = np.mean(gt[viewpoint_frame][:, 14, :], axis=0)
    bottom = np.max(gt[viewpoint_frame][:, :, 1])
